...
const migration = {
    postgres: {
        up: async (db: Kysely<any>) => {
            await db.schema
                .createTable('warnings_config')
                .addColumn('id', 'serial', (col) => col.primaryKey())
                .addColumn('guild_id', 'text', (col) => col.notNull())
                .addColumn('mod_channel_id', 'text', (col) => col.notNull())
            ...
            await db.schema
                .createIndex('warnings_config_guild_unique_idx')
                .on('warnings_config')
                .column('guild_id')
                .unique()
                .execute();
        ...
        down: async (db: Kysely<any>) => {
            await db.schema.dropTable('warnings_config').ifExists().execute();
        },
    },
    sqlite: {
        up: async (db: Kysely<any>) => {
