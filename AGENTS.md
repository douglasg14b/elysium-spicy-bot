...
- `github-plan-cli/` — GitHub “Jarvis” plan + implement automation (`pnpm github-plan`, routed by `.github/workflows/issue-implementation-plan.yml` → `.github/workflows/jarvis-plan.yml` / `jarvis-implement.yml`); kept outside `src/` so it can be split into its own package/repo later. Tests under `github-plan-cli/__tests__/`. Local planner without a token: `pnpm github-plan plan run-local --file <context.md>` (or `--stdin`). **Jarvis CI:** workflows run `scripts/jarvis-cursor-agent-install.sh`, which executes the vendored `scripts/vendor/cursor-com-install.sh` from https://cursor.com/install (replace that file to bump the installer and its embedded agent build). **`GITHUB_PLAN_DEBUG`** is **on by default** (`[github-plan:debug]` on stderr); set repository variable `GITHUB_PLAN_DEBUG` to `0` / `false` / `off` to quiet CI. **`JARVIS_AGENT_TIMEOUT_MS`** in reusable workflows is **1 hour** before a hung Cursor `agent` receives SIGTERM (then SIGKILL). Comment concurrency uses `cancel-in-progress: false` (avoids aborting mid-push); overlapping runs on the same issue can still race on `ai/issue-N` — use spacing between triggers or a GitHub Environment with concurrency if that becomes a problem. `plan implement` fails if the implementer leaves no staged changes after `pnpm build`.
