#!/usr/bin/env bash
# Vendored installer; overwrite to bump the baked-in agent package version. Canonical download URL: ../jarvis-cursor-agent-install.sh (CURSOR_COM_INSTALL_SCRIPT_URL).
...
TEMP_EXTRACT_DIR="$HOME/.local/share/cursor-agent/versions/.tmp-2026.03.25-933d5a6-$(date +%s)"
...
DOWNLOAD_URL="https://downloads.cursor.com/lab/2026.03.25-933d5a6/${OS}/${ARCH}/agent-cli-package.tar.gz"
...
FINAL_DIR="$HOME/.local/share/cursor-agent/versions/2026.03.25-933d5a6"
...
ln -s ~/.local/share/cursor-agent/versions/2026.03.25-933d5a6/cursor-agent ~/.local/bin/agent
ln -s ~/.local/share/cursor-agent/versions/2026.03.25-933d5a6/cursor-agent ~/.local/bin/cursor-agent
