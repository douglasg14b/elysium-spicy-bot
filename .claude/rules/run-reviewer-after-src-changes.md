...
When the active agent has **finished making changes to application TypeScript** under `src/` (e.g. after implementing a feature, fixing a bug, or refactoring), before considering the task done:
...
3. If the report includes **Critical or High** findings, fix them in the current agent, then invoke **`reviewer` again** until no Critical or High findings remain. Medium and Low findings may be fixed at your discretion or left for the user.
