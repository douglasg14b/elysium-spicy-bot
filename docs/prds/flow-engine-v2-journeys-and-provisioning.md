...
- [ ] **Block manifest is the single source of truth**: each block declares, in one module, its `type`, `kind`, `label`, `description`, palette group, icon, config fields, declared outputs, output handles, context requirements, and required bot capabilities. Runtime *and* builder read the same declaration. Extends `nodes/types.ts:26-52`, which today carries only `type`/`kind`/`label`/`configSchema`. (**Not done**)
- [ ] **Config fields are declared, not hand-coded**: a block declares an ordered list of fields, each naming a **control type** from a shared vocabulary (role picker, channel **selector** per §5.4, category picker, text, long text, message/embed composer, number+unit duration, single select, multi select, emoji, colour, resource-binding combobox per §5.7, run-variable reference per §5.3, string list, audience rule per §5.4). Zod remains the authoritative validator; the field list drives the form. (**Not done**)
- [ ] **The control vocabulary is the extension point, not the node type**: bespoke UI is added by registering a new *control type* usable by any block, never by adding a per-node-type branch in the builder. (**Not done**)
- [ ] **Output handles are declared**: a block states its handles (e.g. `true`/`false`, `next`/`timeout`, one per prompt choice) with labels and colours. The builder must not infer handles or kind from the type string — deletes `nodeMeta.ts:88-92` (`kindOf`) and `:100-125` (`branchHandles`). (**Not done**)
- [ ] **Suspension is declared, not special-cased**: a block that parks a run declares that capability and supplies the suspension. Removes the `ACTION_DELAY` / `ACTION_WAIT_FOR_EVENT` string checks at `executor.ts:158` and `:180`. (**Not done**)
- [ ] **Context requirements are declared and validated**: a block states what it needs from the run (e.g. *a ticket*, *a target channel*, *an actor*). Saving a graph that places such a block where the requirement cannot be satisfied fails validation with a message naming the node and the missing requirement — extends `graphValidation.ts` / `nodeDataValidation.ts`. (**Not done**)
- [ ] **Blocks are self-registering by directory**: adding a block does not require editing a hand-maintained array. Replaces the manual list at `nodes/registry.ts:25-40` and the barrel at `nodes/index.ts`. (**Not done**)
- [ ] **Conformance suite**: a single generic test runs against every registered block and fails when the manifest is incomplete, a declared field has no schema counterpart (or vice versa), defaults do not satisfy the schema, a declared handle is unreachable by the executor, a declared output is never written, or the description/summary is missing. (**Not done**)
- [ ] **All 12 existing blocks migrate** to the v2 manifest with no behavior change and no graph migration for saved flows. (**Not done**)
...
- [ ] **Generic inspector**: the config panel renders from the manifest for every block. Deletes the per-type `switch` at `NodeInspector.tsx:181-373`. (**Not done**)
- [ ] **Generic palette and card**: grouping, icon, description, and the one-line node summary come from the server descriptor. Deletes `NODE_EMOJI`, `NODE_DESCRIPTION`, `summarizeNode`, and `defaultDataFor` from `nodeMeta.ts`. (**Not done**)
- [ ] **`GET /api/nodes` returns the full descriptor**, not just `{type, kind, label}` — widens `nodeRoutes.ts:17-21`. Zod schemas remain server-side; what crosses the wire is the field/handle/output descriptor. (**Not done**)
- [ ] **No unknown-block silent fallback**: a node whose type the server does not know renders as an explicit broken node with a named error, never as a generic "no configuration" card. (**Not done**)
...
- [ ] **Subject and actor are distinct**: `FlowRunContext` carries the member the run is about *and* the member who caused the current step, which may differ or be absent. Today there is one `member` (`nodes/types.ts:10-19`) — insufficient for a moderator advancing an applicant's run. (**Not done**)
- [ ] **Run variables**: a run carries a JSON-serialisable named variable bag, readable and writable by blocks, persisted across suspend/resume. (**Not done**)
...
- [ ] **Token vocabulary for copy**: user-facing text supports a small declared token set — at minimum `{{subject.mention}}`, `{{subject.username}}`, `{{actor.mention}}`, `{{guild.name}}`, `{{channel.<resourceKey>}}`, `{{var.<name>}}` — rendered by one shared renderer, with unknown tokens surfaced at save time rather than leaking braces to users. (**Not done**)
- [ ] **Channel context**: the run carries the channel it is operating in, so a block can post "here" (e.g. into the ticket) without an explicit channel picker. (**Not done**)
- [ ] **Widened run persistence**: `flow_runs.contextSnapshot` grows from `{guildId, userId}` (`flowRunsSchema.ts:15-18`) to carry subject, channel, and variables, with an `entityVersion` bump and a migration for in-flight rows. (**Not done**)
...
- [ ] **Random selection from a configured list**: a block picks one item at random from an operator-configured string list and writes it to a declared output. It is **not** bound to messages or embeds — composed with an ordinary post block via the reference picker, it covers picking a prompt, a reviewer, or anything else, and needs no new control type (`string list` is already in the §5.1 vocabulary). Binding randomness to a message payload would force a second block the first time someone wants to pick a non-message. (**Not done**)
...
- [ ] **Prompt lifecycle is tidy**: once a choice is taken (or the run times out, or the run is cancelled), the buttons are disabled or the message updated, so stale controls cannot be pressed later. (**Not done**)
- [ ] **Prompt timeout branch**: a prompt may declare a timeout with its own handle, so an unanswered verification does not park forever. (**Not done**)
...
- [ ] **Ambiguous names disambiguate explicitly**: Discord permits duplicate role names and duplicate channel names across categories. When a typed name matches more than one resource, the operator picks which, showing category/position; the engine never guesses. (**Not done**)
...
- [ ] **A resource may be bound by more than one journey**: unmanaging it under one journey does not unmanage it under another. Two journeys legitimately share a `#welcome`, and install → uninstall → reinstall must not strand a binding. (**Not done**)
...
- [ ] **Drift detection and repair**: if a managed channel is deleted or its permissions are changed outside the engine, the flow reports unhealthy and the operator can re-apply the plan. (**Not done**)
- [ ] **Explicit teardown policy**: deleting or uninstalling a flow never deletes a channel containing history without an explicit, separately confirmed destructive action. Default is unmanage/archive. (**Not done**)
...
- [ ] **A flow may hold many triggers**: one canvas supports several trigger-rooted paths, so an author can lay out an entire journey as a single diagram, or split it across flows, as they prefer. Neither is privileged. (**Not done**)
- [ ] **Every trigger in a flow fires, not just the first**: today both gateway dispatchers select a trigger with `.find()` — [`memberJoinDispatch.ts:28`](../../src/features/flows/engine/memberJoinDispatch.ts) and [`reactionAddDispatch.ts:66`](../../src/features/flows/engine/reactionAddDispatch.ts) — so a second `memberJoin` path on the same canvas is silently dead. All matching triggers must fire, each as its own run. (**Not done**)
- [ ] **Trigger buttons deploy per destination, not per flow**: [`deployFlowButtons.ts:100-101`](../../src/features/flows/logic/deployFlowButtons.ts) posts *every* buttonClick trigger in a flow into one message in one channel, in a single action row. A journey-on-one-canvas needs "Agree to Rules" in `#rules` and "Start Verification" in `#verify-me`, and breaks past 5 buttons. Each button trigger must declare its own destination. (**Not done**)
- [ ] **Disconnected subgraphs are legal and legible**: validation already permits them ([`graphValidation.ts`](../../src/features/flows/engine/graphValidation.ts) has no reachability check); the builder must make separate paths visually clear rather than looking like a broken graph, and must warn about non-trigger nodes that nothing can reach. (**Not done**)
...
- [ ] **Install wizard**: choose names, adopt-or-create each resource, review the plan, apply, then enable. Applying resources and enabling flows are separate steps so a half-configured journey is never live. (**Not done**)
...
- [ ] **Guild-level settings surface**: a server-wide configuration item, editable in the dashboard, holding settings that are not owned by any one feature — starting with **moderator roles**. (**Not done**)
...
- **Q7 — Moderator definition**: promote the tickets feature's `moderationRoles` into a **server-wide settings surface** that tickets, flow audience gates, and future features all read. One definition, migrated up, not duplicated. (§5.12)
