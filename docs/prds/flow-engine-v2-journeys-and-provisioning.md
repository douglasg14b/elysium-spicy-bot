...
- [~] **Drift detection and repair**: if a managed channel is deleted or its permissions are changed outside the engine, the flow reports unhealthy and the operator can re-apply the plan. (**Engine done 2026-09-24; no operator surface yet** — the requirement's own wording is about what an *operator* can do, and today nothing in the dashboard reaches any of this. Carried as open until a surface lands. `resourceDrift.ts` compares a live object against its declaration across four kinds: renamed, reparented, wrong type, and permission overwrites. `journeyDriftPlan.ts` fans it across a journey; `applyDriftRepair.ts` reconciles approved items back to the declaration.
