...
- [ ] **A suspending node with no reachable handle is rejected at save time**: today `executeFlowSegment` (`executor.ts:302-317`) discards the suspension and completes the run instead, so a `action.prompt` node with nothing wired posts its buttons and then writes **no `flow_runs` row at all** — a member pressing one is told the question "may still be setting up", on a question that can never open. Surfaced by step 3 slice D, which deliberately did not fix it there: the check governs every suspending block, and moving it ahead of `run` would change when `action.delay` and `action.waitForEvent` fire their side effects. It belongs here, beside the fan-out rejection, as the other save-time graph guarantee. (**Not done** — see the build order's slice D row for the full reasoning.)
