...
- [x] **Resource binding is one autocomplete field, not an adopt-or-create fork**: for each declared resource the operator types a name into a combobox that suggests matching existing channels/roles. Selecting a suggestion **binds to that existing resource**; typing a name that does not exist **creates it**. Adoption and creation are the same gesture, and the operator is never silently bound to something they did not choose. (**Done 2026-09-24** — the row's **Name** box and its separate *"Does it already exist?"* picker are now one Mantine `Autocomplete`. `Autocomplete` rather than `Select` because a name nothing matches is a good answer meaning "create it", which a `Select` cannot express. Selecting a suggestion adopts; typing anything else renames *and clears an existing adoption in the same patch* — two calls would have silently lost the first, since both derive their next list from the same captured `resources`.)
- [x] **Suggestions are ranked, never auto-applied**: the field pre-fills the journey's default name and surfaces close matches, but a binding is only established by an explicit selection or an explicit new name. (**Done 2026-09-24** — `resourceNameSuggestions.ts`, ranked exact → prefix → substring with no fuzzy tail. `resourceMatchesFilter` rejected fuzzy matching for the filter box as unpredictable; the reasoning holds here for a different reason, recorded in the module: this list is not *narrowing* a set already on screen, it is proposing a **binding**, and a suggestion appearing for reasons the operator cannot reconstruct is one they will accept wrongly or learn to distrust. Ranking strips the label's decoration first — `#`, `@`, and ` · in Support` are things nobody types, so ranking the raw label would make the exact tier unreachable for channels entirely.

    **Never auto-applied, and this is where it was tested.** Auto-adopting on an exact typed match was the obvious way to make a name collision unreachable, and it is what this requirement forbids: typing a name that happens to collide is not the operator choosing that object. Instead the collision raises a **`nameTaken`** chip, and the operator resolves it by picking the suggestion or renaming.

    That chip needed a **new tone**, `blocksInstall`, and the reason is worth recording because it is a real hole the vocabulary had. `resourceChipAgreement.test.ts` holds every `error` chip to being a declaration the *save gate* rejects — and that gate has no guild, so the save genuinely succeeds. But the amber tier is defined as "saves fine, **installs** fine", which a name collision does not. A third state existed with no tier to hold it. `isBlockingChip` deliberately excludes it: an operator part-way through authoring must still be able to save. The agreement test now runs a third block asserting the browser chip against the real `buildInstallPlan` with a stubbed guild, so the claim is checked rather than asserted — sabotage-verified by inverting the comparison and watching *"the browser flags it before the install is attempted"* fail.

    One consequence of the merge, recorded so it is not rediscovered: `nameField` and `adoptPicker` collapse into a single `nameCombobox` jump target. Nothing was lost — **no chip ever pointed at `nameField`**, and the two that pointed at `adoptPicker` are asking for the same control under its new name.)
