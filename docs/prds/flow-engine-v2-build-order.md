...
- **`triggerReactionAdd` declares `requires: ['channel']` but can supply `undefined`.** `asGuildTextChannel` returns `undefined` for an uncached partial message, and save-time validation treats a declaring trigger as a supplier — so a downstream `condition.inChannel` answers "no" because the channel could not be narrowed, not because the run is elsewhere. Narrow (DM reactions are already filtered by the `guild` guard), but the declaration is a contract the runtime can break silently. Slice C should fetch the partial before narrowing. Found during slice B review.
