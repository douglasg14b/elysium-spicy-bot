...
- **postgres**: `set config = jsonb_set(config, '{ticketTypes}', ${sql.lit(typesJson)}::jsonb, true) - 'ticketChannelNameTemplate' - 'userTicketsDeployed' - 'userTicketsDeployedChannelId' - 'userTicketsDeployedMessageId'`, guarded by `where not (config ? 'ticketTypes')` so a re-run is a no-op.

  > **⚠️ Corrected after implementation — this line was a Critical bug.** It originally read `set config = config || ${sql.lit(typesJson)}::jsonb`. `jsonb ||` merges **shallowly at the top level**, and the seed value is the *bare* record `{ support: {…}, verification: {…} }` with no `ticketTypes` wrapper — so `||` would have written `support` and `verification` as **siblings of `modTicketsDeployed`**, left `config.ticketTypes` undefined, never cleared the guard, and made every button on every existing ticket refuse. Silently, and **on postgres only**, because the sqlite arm named its path correctly via `json_set`. That asymmetry is why two review passes read the arms as equivalent when they were not.
  >
  > Use `jsonb_set` with an explicit path: it states where the value lands and cannot merge at the wrong level. Caught during Step A implementation; shipped correct in `9920dcd`, where a unit test pins the payload shape since no postgres runs in CI.
